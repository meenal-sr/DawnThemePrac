...
Step 2: UI Agent       → Spawn with Figma data → writes liquid/scss/component-structure.md
Step 3: Test Agent     → Spawn in ui-only mode → writes ui.spec.ts
        (ui-only)        Based on component-structure.md + test-scenarios.md
Step 4: Run Tests      → Main runs: npx playwright test features/[name]/ui.spec.ts
...
Step 6: TS Agent       → Spawn (no MCP needed) → writes ts + component-api.md
                         SKIP if brief says "No TypeScript needed"
Step 7: Test Agent     → Spawn in full mode → writes functional.spec.ts + integration.spec.ts
        (full)           SKIP if no TS agent ran
...
- Multiple TS agents for independent sections → simultaneously
...
```bash
npx playwright test features/[name]/ui.spec.ts --reporter=list
```
...
2. Send mismatches to UI agent → fixes liquid/scss
...
### Test failure:
1. Route failure to TS agent → fixes TypeScript
...
ui.spec.ts              ← test agent (ui-only)
component-api.md        ← TS agent (if applicable)
functional.spec.ts      ← test agent (full, if applicable)
integration.spec.ts     ← test agent (full, if applicable)
qa/
  visual-qa-report.md   ← visual-qa agent
  figma-*.png           ← main (figma screenshots)
  live-*.png            ← ui.spec.ts (playwright screenshots)
